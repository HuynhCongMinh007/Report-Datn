...
\begin{table}[H]
    ...
    \begin{tabular}{|L{4.4cm}|C{2.4cm}|C{2.4cm}|C{2.4cm}|C{2.4cm}|}
        ...
        Kết nối dịch vụ AI & 81.81\% & 86.20\% & 100.0\% & 50.00\% \\ \hline
        Cổng giao tiếp với trợ lý AI & 67.17\% & 70.57\% & 67.97\% & 50.42\% \\ \hline
        \textbf{Trung bình (8 nhóm chức năng Tài chính)} & \textbf{83.31\%} & \textbf{84.41\%} & \textbf{76.88\%} & \textbf{70.07\%} \\ \hline
    \end{tabular}
    \caption{Độ bao phủ mã nguồn theo nhóm chức năng (phạm vi Phân hệ Quản lý Tài chính)}
    \label{tab:unit-test-coverage}
\end{table}

Các nhóm chức năng lõi của nghiệp vụ 6 hũ (quản lý hũ, lịch chuyển tiền tự động, cảnh báo tài chính, giao dịch tài chính) cùng học bổng và khoản vay đều đạt trên 80\% ở cả bốn chỉ số, cho thấy các nhánh xử lý chính đã được kiểm chứng đầy đủ. Cổng giao tiếp với trợ lý AI có độ bao phủ thấp hơn (67.17\% Statements, 50.42\% Branch) do phần lớn mã nguồn chưa kiểm thử thuộc các luồng phụ trợ (tư vấn nghề nghiệp, lộ trình học tập) nằm ngoài phạm vi Phân hệ Quản lý Tài chính, cùng một số nhánh lỗi hiếm khi xảy ra của tầng tích hợp trực tiếp với dịch vụ AI ngoài khó tái tạo đầy đủ trong môi trường giả lập thuần.
...
\textit{Phương pháp đo:} Bộ dữ liệu kiểm thử gồm 30 hồ sơ sinh viên so khớp với 36 học bổng, có gán nhãn học bổng thực sự phù hợp cho từng hồ sơ. Việc đo được thực hiện ở hai tầng riêng biệt của hệ thống, cả hai đều thực thi cục bộ, không qua mạng hay gọi LLM, đảm bảo kết quả tất định và có thể tái lập:
\begin{itemize}
    \item \textbf{Công thức so khớp:} điểm tương thích kết hợp Jaccard (trọng số 0.5), Bigram (0.3) và SequenceMatcher (0.2) giữa hồ sơ và mô tả học bổng.
    \item \textbf{Pipeline gợi ý sản phẩm:} áp dụng thêm bước lọc theo trường/ngành (gate) rồi sắp xếp kết quả cuối cùng theo mức chênh lệch GPA và giá trị học bổng.
\end{itemize}

Bảng~\ref{tab:scholarship-matching} so sánh kết quả của hai tầng.

\begin{table}[H]
    \centering
    \begin{tabular}{|L{5.0cm}|C{3.0cm}|C{3.0cm}|}
        \hline
        \textbf{Chỉ số} & \textbf{Công thức so khớp} & \textbf{Pipeline gợi ý sản phẩm} \\ \hline
        Top-1 accuracy & \textbf{96.7\%} & 53.3\% \\ \hline
        MRR & 0.983 & 0.683 \\ \hline
        Precision@1 / Recall@1 / F1@1 & 96.7\% / 48.3\% / 64.4\% & 53.3\% / 26.7\% / 35.6\% \\ \hline
        Precision@3 / Recall@3 / F1@3 & 66.7\% / 100.0\% / 80.0\% & 40.0\% / 60.0\% / 48.0\% \\ \hline
        Precision@5 / Recall@5 / F1@5 & 40.0\% / 100.0\% / 57.1\% & 30.7\% / 76.7\% / 43.8\% \\ \hline
        Độ phủ gate trung bình & -- & 100.0\% \\ \hline
    \end{tabular}
    \caption{Kết quả đánh giá gợi ý học bổng (N=30 hồ sơ, 36 học bổng)}
    \label{tab:scholarship-matching}
\end{table}

Công thức so khớp đạt Top-1 accuracy 96.7\% và Recall@3 tuyệt đối 100\% -- cả hai học bổng thực sự liên quan đến mỗi hồ sơ luôn nằm trong top-3 kết quả xếp theo độ tương đồng văn bản. Tuy nhiên, khi đi qua đầy đủ pipeline gợi ý hiển thị cho người dùng (có thêm bước lọc theo trường/ngành và sắp xếp theo GPA-gap/giá trị học bổng), Top-1 accuracy giảm còn 53.3\%: bước lọc theo trường/ngành không loại nhầm học bổng liên quan nào (độ phủ gate 100\%), nhưng thứ tự hiển thị cuối cùng ưu tiên học bổng dễ đạt điều kiện GPA và giá trị cao hơn là mức độ liên quan chủ đề. Đây là khoảng cách đáng ghi nhận giữa năng lực của thuật toán so khớp và thứ tự thực tế người dùng nhìn thấy, gợi ý hướng cải tiến là đưa điểm tương đồng chủ đề vào làm tiêu chí sắp xếp chính thay vì chỉ dùng để hiển thị.
...
\begin{enumerate}
    ...
    \item \textbf{Hiệu quả trợ lực từ AI:} Qua đánh giá có thể thấy phân hệ AI đạt kết quả tốt trên bộ dữ liệu đánh giá, với độ chính xác phân loại giao dịch tự động đạt 98.0\% (Macro-F1 đạt 98.0\%) trên 300 mẫu thử nghiệm, thuật toán so khớp học bổng đạt Top-1 accuracy 96.7\% ở tầng công thức tính điểm tương thích, và trợ lý tư vấn tài chính trả lời thành công 184/184 câu hỏi (100\%) trong bộ kiểm thử mở rộng. Kết quả cũng cho thấy một khoảng cách cần lưu ý giữa năng lực của thuật toán so khớp và thứ tự thực tế hiển thị cho người dùng cuối (Mục~\ref{sec:danh-gia-hoc-bong}), là hướng cải tiến ưu tiên tiếp theo.
