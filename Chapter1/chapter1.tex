...
\begin{itemize}
    ...
    \item \textbf{Tính sẵn sàng (Availability):} Hệ thống hoạt động ổn định trên nền tảng di động và webadmin với cơ chế lưu đệm (Caching) tối ưu, giảm tải cuộc gọi API ngoài. Các thao tác chuyển khoản đơn giữa các hũ được bọc trong transaction để tránh sai lệch số dư; các tác vụ tổng hợp phức tạp hơn (phân bổ thu nhập, xóa hũ) đang được rà soát để bổ sung cơ chế bảo vệ tương đương.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Xác thực JWT và cơ chế phân quyền RolesGuard:} Sử dụng JSON Web Token (JWT) để xác thực người dùng trên các API. Đối với phân hệ Webadmin, hệ thống triển khai bộ phân quyền `RolesGuard` để phân tách quyền truy cập theo vai trò \texttt{ADMIN}, \texttt{MOD} và \texttt{HR} (vai trò \texttt{HR} được tái sử dụng làm vai trò quản trị học bổng của nhà tài trợ, gắn với thực thể \texttt{Organization} của tổ chức tài trợ). Đối với phân hệ Mobile App, sinh viên được xác thực bằng JWT tiêu chuẩn, và mọi truy vấn dữ liệu cá nhân đều được Backend tự động giới hạn theo \texttt{user\_id} trích xuất từ token.
    \item \textbf{Quản lý dữ liệu qua Database Transactions và cơ chế recalculate số dư:} Áp dụng cơ chế \texttt{QueryRunner} Transaction của TypeORM trong NestJS cho các thao tác chuyển khoản giữa các hũ để đảm bảo tính nguyên tử (Atomicity). Đồng thời, xây dựng hàm tính toán lại số dư (`recalculateJarBalance`) dựa trên tổng hợp (\texttt{SUM}) lịch sử giao dịch thực tế trong bảng \texttt{financial\_transaction} thay vì lưu số dư tĩnh, giúp hạn chế sai lệch dữ liệu khi có thao tác điều chỉnh.
