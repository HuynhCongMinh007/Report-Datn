/**
 * Nhóm 2 — Tìm kiếm và lọc (giao dịch, học bổng) — Bảng 5.1.
 * Chạy độc lập, mặc định nhắm vào backend ĐÃ HOST.
 *
 * Endpoint & phân bổ (~100 request):
 *   - GET /finance/transactions (có filter type/paging)  x34
 *   - GET /scholarships (danh sách)                       x33
 *   - GET /universities/dropdown (tra cứu/lọc trường)      x33
 *
 * Usage:
 *   EVAL_USER_EMAIL=... EVAL_USER_PASSWORD=... \
 *   npx ts-node -r tsconfig-paths/register test/load/group-02-tim-kiem-loc.ts
 */
...
const OUT_DIR = process.env.BENCH_OUT_DIR || path.join(__dirname, 'group-02-results');
...
async function timeCall(endpoint: string, iteration: number, fn: () => Promise<unknown>): Promise<Sample> {
  ...
  try {
    ...
    return { iteration, endpoint, ok: false, status: err?.response?.status, latencyMs: Date.now() - start, error: err?.message || String(err) };
  }
}
...
async function main() {
  ...
  console.log(`[Nhóm 2] Đăng nhập ${EMAIL} tại ${BASE_URL} ...`);
  ...
  const filters = [
    {}, { type: 'expense' }, { type: 'income' }, { page: 1, limit: 10 }, { page: 2, limit: 10 },
  ];

  console.log('[Nhóm 2] GET /finance/transactions (filter) x34 ...');
  for (let i = 1; i <= 34; i++) {
    const params = filters[i % filters.length];
    samples.push(await timeCall('GET /finance/transactions', i, () => axios.get(`${BASE_URL}/finance/transactions`, { headers: auth, params })));
    process.stdout.write('.');
  }
  console.log('');

  console.log('[Nhóm 2] GET /scholarships x33 ...');
  for (let i = 1; i <= 33; i++) {
    samples.push(await timeCall('GET /scholarships', i, () => axios.get(`${BASE_URL}/scholarships`, { headers: auth })));
    process.stdout.write('.');
  }
  console.log('');

  console.log('[Nhóm 2] GET /universities/dropdown x33 ...');
  for (let i = 1; i <= 33; i++) {
    samples.push(await timeCall('GET /universities/dropdown', i, () => axios.get(`${BASE_URL}/universities/dropdown`, { headers: auth })));
    process.stdout.write('.');
  }
  console.log('');
  ...
  console.log('\n=== TỔNG HỢP NHÓM 2 — TÌM KIẾM VÀ LỌC ===');
  ...
  fs.writeFileSync(path.join(OUT_DIR, `summary-${ts}.md`), `# Nhóm 2 — Tìm kiếm và lọc\n\nBackend: ${BASE_URL}\nThời gian: ${new Date().toISOString()}\n\n${lines.join('\n')}\n`, 'utf8');
  console.log(`\nĐã lưu vào ${OUT_DIR}`);
}
