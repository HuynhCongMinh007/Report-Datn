...
_TEST_TOKEN = jwt.encode({"iss": "s360-backend", "exp": 9999999999}, TEST_SECRET, algorithm="HS256")
AUTH_HEADER = {"Authorization": f"Bearer {_TEST_TOKEN}"}
...
async def test_stream_emits_no_token_events_when_blocked():
    """Khi request bị block, vẫn có token event chứa thông báo cảnh báo nhưng không có status/agent events."""
    mock_agent = MagicMock()
    mock_agent.astream = AsyncMock()
    ...
    assert len(token_events) == 1, "Phải có đúng 1 token event khi bị block chứa thông báo cảnh báo"
    assert "Thực thi một chạm" in token_events[0]["data"]["text"]
...
@pytest.mark.asyncio
async def test_retry_after_enable_actions_toggle():
    """Khi người dùng bật enable_actions và gửi 'Tôi đã bật rồi, thực thi lại giúp tôi', agent sẽ được gọi thành công."""
    async def fake_astream(*args, **kwargs):
        yield ("token", "Tôi sẽ thực hiện các hành động sau cho bạn:")
        yield ("actions", json.dumps([{
            "type": "CREATE_TRANSACTION",
            "title": "Ghi nhận 50k ăn sáng",
            "description": "Thêm giao dịch 50,000đ vào lọ Thiết yếu",
            "params": {"amount": 50000, "category": "Ăn uống"}
        }]))
    ...
    with patch("app.api.finance.chat.get_finance_agent", return_value=mock_agent), \
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
                "/api/v1/chat/stream",
                json={
                    "user_id": "user-test-retry",
                    "message": "Tôi đã bật rồi, thực thi lại giúp tôi",
                    "enable_actions": True,
                },
                headers=AUTH_HEADER,
            )
    ...
    done_events = [e for e in events if e.get("event") == "done"]
    assert done_events
    assert done_events[0]["data"]["actionHint"] is True
    assert len(done_events[0]["data"]["actions"]) == 1
    assert done_events[0]["data"]["actions"][0]["type"] == "CREATE_TRANSACTION"
